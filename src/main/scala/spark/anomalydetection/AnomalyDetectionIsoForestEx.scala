...
object AnomalyDetectionIsoForestEx {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val df = spark.read.format("csv")
      ...
      .schema(schema)
      .load("src/main/resources/data/tshark.csv")
      .cache()
    df.printSchema()
    ...
    val cond = tDf("ip_src") === tDf("ip_src") && tDf("frame_protocols") === tDf("frame_protocols") && tDf("frame_time") >= gDf("start") && tDf("frame_time") <= gDf("end")
    val jDf = tDf
      .join(gDf, cond, "left")
    ...
    vDf.filter(col("predicted_label")===1.0).show(truncate = false)
